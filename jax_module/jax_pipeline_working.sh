...
if [[ "'"$INPUT"'" == *.cram ]]; then
    ...
    samtools view -T "'"$REF"'" -b -o "'"$UNSORTED_BAM"'" "'"$INPUT"'"
else
...
jellyfish count -m 25 -s 100M -t '"$THREADS"' -C -o "'"$JELLY_DB"'" "'"$REF"'"

echo "[Step 5/6] Dump .jf to .kmer FASTA..."
echo "  Dumping kmer counts to '\'''"$KMER_OUT"''\''"
JAX-CNV GrabJellyfishKmer --ascii -i "'"$JELLY_DB"'" -f "'"$REF"'" -o "'"$KMER_OUT"'"

echo "[Step 6/6] Run JAX-CNV..."
echo "  Analyzing CNV signal: '\'''"$SORTED_BAM"''\'' -> '\'''"$BED_OUT"''\'' and '\'''"$LOG_OUT"''\''"
JAX-CNV GetCnvSignal -k "'"$KMER_OUT"'" -f "'"$REF"'" -b "'"$SORTED_BAM"'" -o "'"$BED_OUT"'" --log "'"$LOG_OUT"'"
