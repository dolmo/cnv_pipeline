...
include { JAX_CNV } from '../../modules/local/jax_cnv' 

include { BED_TO_VCF } from '../../modules/local/bed_to_vcf'
...
include { ANNOTATE_CARDIOMYOPATHY_GENES} from '../../modules/local/annotate_cardio_genes'
include { CALCULATE_CNV_BURDEN         } from '../../modules/local/calculate_cnv_burden'
include { CNV_POPULATION_ANALYSIS      } from '../../modules/local/cnv_population_analysis'
include { ANNOSV                       } from '../../modules/local/annosv'
...
// ---- JAX_CNV wiring with BED to VCF conversion ----
if (params.tools && params.tools.split(',').contains('jax_cnv')) {

    log.info "JAX_CNV tool detected - proceeding with setup"
    log.info "JAX_CNV will use optimized parameters for large datasets"
    
    // Use the cram_variant_calling channel with metadata preserved
    ch_jax_input = cram_variant_calling
        .filter { meta, cram, crai -> 
            log.info "Filtering sample: ${meta.id}, status: ${meta.status}"
            return meta.status == 0 || meta.status == 1 
        }
        .map { meta, cram, crai -> 
            log.info "Processing sample: ${meta.id}, cram: ${cram}"
            return tuple(meta, cram)
        }

    // Reference files
    ch_ref = Channel.value(file(params.fasta))
    ch_ref_fai = params.fasta_fai ? 
        Channel.value(file(params.fasta_fai)) : 
        Channel.empty()

    // Wrapper script
    def wrapperScript = params.jax_script ?: 'cnv_pipeline/jax_module/jax_pipeline_working.sh'
    ch_script = file(wrapperScript).exists() ? 
        Channel.value(file(wrapperScript)) : 
        Channel.empty()

    // Call JAX_CNV process
    JAX_CNV(ch_jax_input, ch_ref, ch_ref_fai, ch_script)
    ...
    // Now you have VCF files that can be used by downstream annotation processes
    ch_jax_vcf = BED_TO_VCF.out.vcf

    // Collect reports for MultiQC
    reports = reports.mix( JAX_CNV.out.cnv_log )
    reports = reports.mix( BED_TO_VCF.out.vcf.map { meta, vcf -> vcf } )

    // Gather versions
    versions = versions.mix( JAX_CNV.out.versions )
    versions = versions.mix( BED_TO_VCF.out.versions )
}

if (params.tools && params.tools.split(',').contains('jax_cnv')) {
    
    // CNV Filtering (optional)
    if (params.filter_cnvs) {
        log.info "Applying CNV size and quality filters"
        
        FILTER_CNV_SIZE(
            BED_TO_VCF.out.vcf, 
            params.min_cnv_size,    // Make sure this parameter exists
            params.max_cnv_size     // Make sure this parameter exists
        )
        
        FILTER_CNV_QUALITY(FILTER_CNV_SIZE.out.filtered_vcf)
        
        ch_filtered_cnvs = FILTER_CNV_QUALITY.out.quality_filtered_vcf
        versions = versions.mix(FILTER_CNV_SIZE.out.versions)
        versions = versions.mix(FILTER_CNV_QUALITY.out.versions)
    } else {
        ch_filtered_cnvs = BED_TO_VCF.out.vcf
    }
    
      // CNV Gene Annotation (optional)
    if (params.annotate_cardio_genes) {
        log.info "Annotating CNVs with cardiomyopathy genes"
        
        // Use GTF if available, otherwise pass empty channel
        ch_gtf = params.gtf ? Channel.value(file(params.gtf)) : Channel.value([])
        
        // Fix the cardiomyopathy genes file path
        ch_cardio_genes = params.cardiomyopathy_genes ? 
            Channel.value(file(params.cardiomyopathy_genes, checkIfExists: true)) : 
            Channel.empty()
        
        ANNOTATE_CARDIOMYOPATHY_GENES(
            ch_filtered_cnvs, 
            ch_cardio_genes,  // Use the channel instead of params directly
            ch_gtf
        )
        
        ch_final_cnvs = ANNOTATE_CARDIOMYOPATHY_GENES.out.annotated_vcf
        versions = versions.mix(ANNOTATE_CARDIOMYOPATHY_GENES.out.versions)
    } else {
        ch_final_cnvs = ch_filtered_cnvs
    }
    

    // 🆕 ADD THIS ANNOSV SECTION RIGHT HERE
    if (params.annotate_with_annosv) {
        log.info "Annotating CNVs with AnnoSV for clinical interpretation"
        
        ANNOSV(ch_final_cnvs)
        
        ch_final_cnvs = ANNOSV.out.annotated_vcf
        versions = versions.mix(ANNOSV.out.versions)
        reports = reports.mix(ANNOSV.out.annotated_tsv.map { meta, tsv -> tsv })
    }
    // END OF ANNOSV SECTION
    
       // CNV Burden Analysis (optional)
    if (params.cnv_burden_analysis) {
        log.info "Performing CNV burden analysis"
        
        CALCULATE_CNV_BURDEN(ch_final_cnvs)
        
        // Population-level analysis (collect all samples)
        // FIX: Extract just the burden files, not the metadata
        ch_all_burden = CALCULATE_CNV_BURDEN.out.burden_stats
            .map { meta, burden_file -> burden_file }  // Extract just the file
            .collect()  // Collect all burden files
        
        if (params.sample_metadata) {
            CNV_POPULATION_ANALYSIS(ch_all_burden, file(params.sample_metadata))
            versions = versions.mix(CNV_POPULATION_ANALYSIS.out.versions)
            reports = reports.mix(CNV_POPULATION_ANALYSIS.out.association_results)
        }
        
        versions = versions.mix(CALCULATE_CNV_BURDEN.out.versions)
        reports = reports.mix(CALCULATE_CNV_BURDEN.out.burden_stats.map { meta, stats -> stats })
    }
    }
    ...
    if (params.tools && params.tools.split(',').contains('jax_cnv')) {
        // Transform CNV VCFs to match Sarek's annotation channel structure
        ch_jax_vcf_for_annotation = ch_final_cnvs.map { meta, vcf ->
            def new_meta = meta.clone()
            new_meta.variantcaller = 'jax_cnv'
            new_meta.tool = 'jax_cnv'
            return tuple(new_meta, vcf)
        }
        
        vcf_to_annotate = vcf_to_annotate.mix(ch_jax_vcf_for_annotation)
    }
